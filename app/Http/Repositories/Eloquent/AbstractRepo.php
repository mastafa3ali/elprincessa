...
class AbstractRepo implements AbstractRepoInterface
{
    ...
    public function mediastore($file,$destination_path)
    {
        $fileName = time() . rand(0, 999999999) . '.' . $file->getClientOriginalExtension();
        $path = $file->storeAs('public/'.$destination_path, $fileName);
        return [
            'user_id' => auth()->id(),
            'filename' => $fileName,
            'path' => $path,
    ...
    public function mediadestroy($ids)
    {
        FacadesFile::find(explode(',', $ids))->each->delete();
    }
